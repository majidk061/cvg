	...
	<style>
        ...
        <div class="blog-area" style="margin-bottom:40px;">
        	<form method="get" enctype="multipart/form-data" id="load_news_form">
                ...
                <div class="row">
                	 <div class="col-12 col-lg-3 spaceright-0 animated bounceInLeft">
                     	@include('layouts.sidebar_news')
                     </div>
                     <div class="col-12 col-lg-9 ">
                     	<div class="col-12 spaceright-0">
                        	<div class="row">
                            	<div class="toolbar mb-3 animated bounceInUp">
                                ...
                                <div class="blogs blogs-4x" id="listing-news">
                                    ...
                                    @foreach($result['news']['news_data'] as $key=>$news_data)
                                        ...
                                        <div class="blog-post animated jackInTheBox">
                                            <article>
...
$(document).ready(function() {
    ...
    $(document).on('click', '#load_news', function(e){ 
        ...
        var formData = $("#load_news_form").serialize();
        $.ajax({
            ...
            success: function (res) {
                if($.trim().res==0){                       
                    ...
                    if(total_record<showing_record){
                        $('#load_news').hide();
                        $('.showing_record').html(total_record);
                    }else{
                        $('.showing_record').html(showing_record);
                    }
                }
                $('#loader').hide();
            },
    ...
    //sortbynews
    $(document).on('change', '.sortbynews', function(e){   
        $('#loader').css('display','flex');
        $("#load_news_form").submit();
    });
    // This WILL work because we are listening on the 'document', 
    // for a click on an element with an ID of #test-element
    $(document).on('click','#list_news',function(){ 
    console.log("==================?");       
        if (!$(this).hasClass('active')) {
            $('#listing-news, .load-more-area').hide();        
            $( '#listing-news' ).removeClass( 'blogs-4x' );
            $( '#listing-news' ).addClass( 'blogs-list' );
            $( '#grid_news' ).removeClass( 'active' );
            $( this ).addClass( 'active' );        
            $('#listing-news, .load-more-area').fadeIn(1000);  
        }
    });

    $(document).on('click','#grid_news',function(){    
        if (!$(this).hasClass('active')){      
            $('#listing-news, .load-more-area').hide();    
            $( '#listing-news' ).removeClass( 'blogs-list' );
            $( '#listing-news' ).addClass( 'blogs-4x' );
